...
_REQUIRED_V3_KEYS = (
    ...
    "kim_trend_filter",
    "use_momentum_filter",
    "min_liquidity_market_cap_krw",
    "min_liquidity_trade_amount_krw",
)
...
def pullback_scan_params_from_yaml_section(v3: dict) -> PullbackScanParams:
    ...
    return PullbackScanParams(
        ...
        min_liquidity_market_cap_krw=float(v3["min_liquidity_market_cap_krw"]),
        min_liquidity_trade_amount_krw=float(v3["min_liquidity_trade_amount_krw"]),
    )
...
def _overlay_pullback_scan_params(
    ...
    liq_cap = base.min_liquidity_market_cap_krw
    if (
        "min_liquidity_market_cap_krw" in data
        and data["min_liquidity_market_cap_krw"] is not None
    ):
        liq_cap = float(
            str(data["min_liquidity_market_cap_krw"]).replace(",", "")
        )

    liq_trd = base.min_liquidity_trade_amount_krw
    if (
        "min_liquidity_trade_amount_krw" in data
        and data["min_liquidity_trade_amount_krw"] is not None
    ):
        liq_trd = float(
            str(data["min_liquidity_trade_amount_krw"]).replace(",", "")
        )
    ...
    return PullbackScanParams(
        ...
        min_liquidity_market_cap_krw=liq_cap,
        min_liquidity_trade_amount_krw=liq_trd,
    )
