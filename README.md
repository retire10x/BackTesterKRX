...
(화면: **시장·종목·검색** 및 **스크리너 모드**(전체 / 스크리너 / 시총 상위 / 돌파 에너지 / 당일 타점 추적)·검색 결과(티커|종목명|시총, **당일 타점 모드**에서는 `| 신호경과일 | 이격도`), 기간 기본 **6개월 전~오늘**. 우측 차트는 창에 맞춰 **가변 확장**(최소 크기 약 **960×540**, 폴백 **1280×840**)·실측 `chart_overlay_host` 기준 PNG 리사이즈. 매매 기준 이평 **5·10·20**, 추세선 **20·120** 기본, 캘린더 선택.)  
...
| `src/stock_screener.py` | 일봉 스크리너(ATR14 고정·MA120 역배열 하드 필터·거래대금·낙폭·거래량건조 순위 융합·시총 상위·돌파 에너지·골든+진입 필터 전환 추적(v4.12)·GUI·CLI 공용) |
